...
//     //       // Sum each individual stat across all players in the team
//     //       totalFTM: { $sum: "$teams.players.stats.FTM" },
//     //       totalFTA: { $sum: "$teams.players.stats.FTA" },
//     //       totalPTS: { $sum: "$teams.players.stats.PTS" },
//     //       total2PM: { $sum: "$teams.players.stats.2PM" }, // 2PT Made
//     //       total2PA: { $sum: "$teams.players.stats.2PA" }, // 2PT Attempted
//     //       total3PM: { $sum: "$teams.players.stats.3PM" }, // 3PT Made
//     //       total3PA: { $sum: "$teams.players.stats.3PA" }, // 3PT Attempted
//     //       totalFG_PERCENT: { $sum: "$teams.players.stats.FG_PERCENT" },
//     //       totalPM: { $sum: "$teams.players.stats.PM" },
//     //       totalAST: { $sum: "$teams.players.stats.AST" },
//     //       totalTO: { $sum: "$teams.players.stats.TO" },
//     //       totalPF: { $sum: "$teams.players.stats.PF" },
//     //       totalOREB: { $sum: "$teams.players.stats.OREB" },
//     //       totalREB: { $sum: "$teams.players.stats.REB" },
//     //       totalDREB: { $sum: "$teams.players.stats.DREB" },
//     //       totalBLK: { $sum: "$teams.players.stats.BLK" },
//     //       totalSTL: { $sum: "$teams.players.stats.STL" },
//     //       totalTEMPMIN: { $sum: "$teams.players.stats.TEMPMIN" },
...
//     //   // Group the stats once more (if necessary) to calculate the total for the team (in case stats are split across multiple documents)
//     //   {
//     //     $group: {
//     //       _id: "$_id",
//     //       teamName: { $first: "$teamName" },
...
//     //       total3PM: { $sum: "$total3PM" }, // Sum 3PM
//     //       total3PA: { $sum: "$total3PA" }, // Sum 3PA
//     //       totalFG_PERCENT: { $sum: "$totalFG_PERCENT" },
//     //       totalPM: { $sum: "$totalPM" },
//     //       totalAST: { $sum: "$totalAST" },
//     //       totalTO: { $sum: "$totalTO" },
//     //       totalPF: { $sum: "$totalPF" },
//     //       totalOREB: { $sum: "$totalOREB" },
//     //       totalREB: { $sum: "$totalREB" },
//     //       totalDREB: { $sum: "$totalDREB" },
//     //       totalBLK: { $sum: "$totalBLK" },
//     //       totalSTL: { $sum: "$totalSTL" },
//     //       totalTEMPMIN: { $sum: "$totalTEMPMIN" },
...
//     //   // Calculate additional stats like percentages and points
//     //   {
//     //     $project: {
//     //       teamName: 1,
//     //       teamLogo: 1,
...
//     //           then: 0,
//     //           else: {
//     //             $multiply: [{ $divide: ["$totalAST", "$totalMIN"] }, 100],
//     //           },
//     //         },
//     //       },
//     //       STL: {
...
//     //           then: 0,
//     //           else: {
//     //             $multiply: [{ $divide: ["$totalSTL", "$totalMIN"] }, 100],
//     //           },
//     //         },
//     //       },
//     //       BLK: {
...
//     //           then: 0,
//     //           else: {
//     //             $multiply: [{ $divide: ["$totalBLK", "$totalMIN"] }, 100],
//     //           },
//     //         },
//     //       },
//     //       TO: {
...
//     //           then: 0,
//     //           else: {
//     //             $multiply: [{ $divide: ["$totalTO", "$totalMIN"] }, 100],
//     //           },
//     //         },
//     //       },
//     //       PE: {
...
//     //           then: 0,
//     //           else: {
//     //             $multiply: [{ $divide: ["$totalPF", "$totalMIN"] }, 100],
//     //           },
//     //         },
//     //       },
//     //     },
...
//       // Calculate additional stats like percentages and points, and round to 1 decimal place
...
//           // Calculate additional percentages and round to 1 decimal place
//           OREB: {
//             $round: [
//               {
//                 $cond: {
...
// @route    POST api/games/
// @desc     Get all game by sport type
// @access   Public
...
// @route    POST api/games/gameId/status/:palyerId
...
// @route    POST api/games/undo/:gameId
// @desc     Undo the last game action
// @access   Public
...
//     if (!game.undoEnable) return res.status(400).json({ message: "Undo is not allowed" });
...
//     const team = game.teams.find(team => team.team.toString() === teamId.toString());
//     if (!team) {
//       return res.status(404).json({ message: "Team not found in this game" });
//     }

//     const playerStat = team.players.find(p => p.player.toString() === playerId.toString());
//     if (!playerStat) {
//       return res.status(404).json({ message: "Player not found in this team" });
//     }
...
//         team.players.forEach(player => {
//           const startTime = new Date(player.stats?.get("startTime"));
//           const endTime = game.gameAction[game.gameAction.length - 1].time;
//           const minutes = Math.floor((endTime - startTime) / 1000 / 60);
//           player.stats?.set("TEMPMIN", (player.stats?.get("MIN") || 0) + minutes);
//         });
//       });
...
//     switch (action) {
//       case "point1":
//         playerStat.stats.set("FTM", Math.max(0, playerStat.stats.get("FTM") - 1));
//         playerStat.stats.set("FTA", Math.max(0, playerStat.stats.get("FTA") - 1));
//         playerStat.stats.set("PTS", Math.max(0, playerStat.stats.get("PTS") - 1));
//         team.stats.set(`Q${game.currentTurn}`, Math.max(0, team.stats.get(`Q${game.currentTurn}`) - 1));
//         team.stats.set("SCORE", Math.max(0, team.stats.get("SCORE") - 1));
//         playerStat.stats.set("PM", Math.max(0, playerStat.stats.get("PM") - 1));

//         team.players.forEach(teamPlayer => {
//           teamPlayer.stats.set("PM", Math.max(0, teamPlayer.stats.get("PM") - 1));
//         });

//         game.teams.forEach(opponentTeam => {
//           if (opponentTeam.team.toString() !== teamId.toString()) {
//             opponentTeam.players.forEach(opponentPlayer => {
//               opponentPlayer.stats.set("PM", (opponentPlayer.stats.get("PM") || 0) + 1);
//             });
//           }
//         });
//         break;
...
//       case "point2":
//         playerStat.stats.set("FGM", Math.max(0, playerStat.stats.get("FGM") - 1));
//         playerStat.stats.set("FGA", Math.max(0, playerStat.stats.get("FGA") - 1));
//         playerStat.stats.set("PTS", Math.max(0, playerStat.stats.get("PTS") - 2));
//         team.stats.set(`Q${game.currentTurn}`, Math.max(0, team.stats.get(`Q${game.currentTurn}`) - 2));
//         team.stats.set("SCORE", Math.max(0, team.stats.get("SCORE") - 2));
//         playerStat.stats.set("PM", Math.max(0, playerStat.stats.get("PM") - 2));
//         break;

//       case "point3":
//         playerStat.stats.set("3PM", Math.max(0, playerStat.stats.get("3PM") - 1));
//         playerStat.stats.set("3PA", Math.max(0, playerStat.stats.get("3PA") - 1));
//         playerStat.stats.set("PTS", Math.max(0, playerStat.stats.get("PTS") - 3));
//         team.stats.set(`Q${game.currentTurn}`, Math.max(0, team.stats.get(`Q${game.currentTurn}`) - 3));
...
//         playerStat.stats.set("FT_PERCENT", ((playerStat.stats.get("FTM") / fta) * 100).toFixed(2));
//         break;

//       case "miss2":
//         playerStat.stats.set("FGA", Math.max(0, playerStat.stats.get("FGA") - 1));
//         const fga = playerStat.stats.get("FGA") || 1;
//         playerStat.stats.set("FG_PERCENT", ((playerStat.stats.get("FGM") / fga) * 100).toFixed(2));
//         break;

//       case "miss3":
//         playerStat.stats.set("3PA", Math.max(0, playerStat.stats.get("3PA") - 1));
//         const threePA = playerStat.stats.get("3PA") || 1;
//         playerStat.stats.set("3P_PERCENT", ((playerStat.stats.get("3PM") / threePA) * 100).toFixed(2));
//         break;

//       case "AST":
//         playerStat.stats.set("AST", Math.max(0, playerStat.stats.get("AST") - 1));
//         break;
...
//       case "REB_OFEN":
//         playerStat.stats.set("OREB", Math.max(0, playerStat.stats.get("OREB") - 1));
//         playerStat.stats.set("REB", Math.max(0, playerStat.stats.get("REB") - 1));
//         break;

//       case "REB_DEFE":
//         playerStat.stats.set("DREB", Math.max(0, playerStat.stats.get("DREB") - 1));
//         playerStat.stats.set("REB", Math.max(0, playerStat.stats.get("REB") - 1));
//         break;

//       case "BLK":
//         playerStat.stats.set("BLK", Math.max(0, playerStat.stats.get("BLK") - 1));
//         break;

//       case "STL":
//         playerStat.stats.set("STL", Math.max(0, playerStat.stats.get("STL") - 1));
//         break;
//     }
...
//       return res.status(404).json({ message: "Game not found" });
//     }
//     if (!game.undoEnable) return res.json({ message: "You can not Undo" });
//     const lastAction = game.gameAction.pop();
//     if (!lastAction) {
//       return res.status(400).json({ message: "No actions to undo" });
//     }
...
//           player.stats?.set(
//             "TEMPMIN",
//             (player.stats?.get("MIN") || 0) + minutes
//           );
//         });
//       });
//     } else {
...
//       });
//     }
//     // Revert the changes based on the action type and data
//     switch (action) {
//       case "point1":
//         playerStat.stats?.set("FTM", playerStat.stats?.get("FTM") - 1);
//         playerStat.stats?.set("FTA", playerStat.stats?.get("FTA") - 1);
//         playerStat.stats?.set("PTS", playerStat.stats?.get("PTS") - 1);
//         team.stats.set(
//           `Q${game.currentTurn}`,
//           team.stats.get(`Q${game.currentTurn}`) - 1
//         );
//         team.stats?.set("SCORE", team.stats?.get("SCORE") - 1);
//         playerStat.stats?.set("PM", playerStat.stats?.get("PM") - 1);
//         team.players.forEach((teamPlayer) => {
//           teamPlayer.stats?.set("PM", (teamPlayer.stats?.get("PM") || 0) - 1);
//         });
//         game.teams.forEach((opponentTeam) => {
...
//         break;
//       case "point2":
//         playerStat.stats?.set("FGM", playerStat.stats?.get("FGM") - 1);
//         playerStat.stats?.set("FGA", playerStat.stats?.get("FGA") - 1);
//         playerStat.stats?.set("PTS", playerStat.stats?.get("PTS") - 2);
//         team.stats.set(
//           `Q${game.currentTurn}`,
//           team.stats.get(`Q${game.currentTurn}`) - 2
//         );
//         team.stats?.set("SCORE", team.stats?.get("SCORE") - 2);
//         playerStat.stats?.set("PM", playerStat.stats?.get("PM") - 2);
//         team.players.forEach((teamPlayer) => {
//           teamPlayer.stats?.set("PM", (teamPlayer.stats?.get("PM") || 0) - 2);
//         });
//         game.teams.forEach((opponentTeam) => {
...
//         break;
//       case "point3":
//         playerStat.stats?.set("3PM", playerStat.stats?.get("3PM") - 1);
//         playerStat.stats?.set("3PA", playerStat.stats?.get("3PA") - 1);
//         playerStat.stats?.set("PTS", playerStat.stats?.get("PTS") - 3);
//         team.stats.set(
//           `Q${game.currentTurn}`,
//           team.stats.get(`Q${game.currentTurn}`) - 3
//         );
//         team.stats?.set("SCORE", team.stats?.get("SCORE") - 3);
//         playerStat.stats?.set("PM", playerStat.stats?.get("PM") - 3);
//         team.players.forEach((teamPlayer) => {
//           teamPlayer.stats?.set("PM", (teamPlayer.stats?.get("PM") || 0) - 3);
//         });
//         game.teams.forEach((opponentTeam) => {
...
//         break;
//       case "AST":
//         playerStat.stats?.set("AST", playerStat.stats?.get("AST") - 1);
//         break;
//       case "TO":
//         playerStat.stats?.set("TO", playerStat.stats?.get("TO") - 1);
//         break;
//       case "FOUL":
//         playerStat.stats?.set("PF", playerStat.stats?.get("PF") - 1);
//         break;
//       case "REB_OFEN":
//         playerStat.stats?.set("OREB", playerStat.stats?.get("OREB") - 1);
//         playerStat.stats?.set("REB", playerStat.stats?.get("REB") - 1);
//         break;
//       case "REB_DEFE":
//         playerStat.stats?.set("DREB", playerStat.stats?.get("DREB") - 1);
//         playerStat.stats?.set("REB", playerStat.stats?.get("REB") - 1);
//         break;
//       case "BLK":
//         playerStat.stats?.set("BLK", playerStat.stats?.get("BLK") - 1);
//         break;
//       case "STL":
//         playerStat.stats?.set("STL", playerStat.stats?.get("STL") - 1);
//         break;
//     }
//     game.undoEnable = false;
//     // Remove the last action from the GameActions collection
//     await game.save();

//     // Send a success respons    "message": "lastAction.remove is not a function"
//     res.status(200).json(game);
//   } catch (error) {
//     res.status(400).json({ message: error.message });
//   }
