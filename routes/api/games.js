...
router.post("/timeout/:gameId/:teamId", auth, async (req, res) => {
  try {
    ...
    const team = game.teams.find(t => t.team.toString() === req.params.teamId);
...
router.post("/finish/:gameId", auth, async (req, res) => {
  ...
  try {
    ...
    // ✅ Iterate teams & players
    for (const team of game.teams) {

      for (const player of team.players) {

        // ✅ Only count time if player is still in court
        if (player.isInCourt && player.stats?.get("startTime")) {
      ...
      // ✅ Team overtime calculation (unchanged logic, but safe)
      if (game.overTimeStart) {
        const otStart = moment(game.overTimeStart);
        const otMinutes = endTime.diff(otStart, "minutes");
        if (!isNaN(otMinutes) && otMinutes > 0) {
          team.stats.set(
            "OT",
            (team.stats.get("OT") || 0) + otMinutes
          );
        }
      }
    }
    ...
    if (game.tournamentId) {
      resultData.tournamentId = game.tournamentId;
    }

    await GameResult.create([resultData], { session });
...
const stopPlayerTime = (player) => {
  if (!player.stats) return;

  const startTime = player.stats.get("startTime");
  if (!startTime) return;

  const minutes = moment().diff(moment(startTime), "minutes");

  player.stats.set(
    "MIN",
    (player.stats.get("MIN") || 0) + Math.max(minutes, 0)
  );

  player.stats.delete("startTime");
};

const stopAllPlayers = (game) => {
  game.teams.forEach(team => {
    team.players.forEach(player => {
      stopPlayerTime(player);
    });
